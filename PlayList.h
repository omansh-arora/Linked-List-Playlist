...
private:
	...
	void copy(const PlayList & pl)
	{

		if(pl.top!=nullptr){
			top = new PlayListNode(pl.top->song,nullptr);
			PlayListNode* topTemp = top;
			PlayListNode* temp = pl.top;
			lstSize = pl.lstSize;
			while(temp->next!=nullptr){

				topTemp -> next = new PlayListNode(temp->next->song);
				temp = temp->next;
